...
public class RentManagementImplements extends UnicastRemoteObject implements RentManagement {
    ...
    public boolean addRequest(Car c, Requests request) throws RemoteException {
        Queue<Requests> queue = requests.get(c.getMatriculeCar());
        // check if employee already request this car
        if (queue.stream().filter(data -> {
            try {
                return data.getEmployee() == request.getEmployee();
            } catch (RemoteException e) {
                throw new IllegalArgumentException();
            }
        }).count() > 0) {
        ...
        if(rentedCars.entrySet()
                ...
                .anyMatch(data -> {
                    try {
                        return data.getKey().equals(c.getMatriculeCar())
                                && data.getValue().getEmployee() == request.getEmployee();
                    } catch (RemoteException e) {
                        throw new IllegalArgumentException();
                    }
                })
